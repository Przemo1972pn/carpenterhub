      ...
      <div className="max-w-[1200px] mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {services.map((s, i) => (
            <div
              key={i}
              className="bg-gradient-to-br from-[#2D5016] to-[#4A7C2E] text-white p-10 rounded-xl transition-all duration-300 hover:scale-105 hover:shadow-2xl"
            >
              <h3 className="text-2xl mb-4 font-bold">{s.icon} {s.title}</h3>
              <p className="mb-5 leading-relaxed">{s.desc}</p>
              <ul className="list-none">
                {s.items.map((item, j) => (
                  <li key={j} className="py-2 pl-6 relative before:content-['✓'] before:absolute before:left-0 before:text-[#D4AF37] before:font-bold">
                    {item}
                  </li>
                ))}
              </ul>
